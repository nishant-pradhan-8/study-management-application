...
const deleteNotes = async (req, res) => { 
  try {
    ...
    if (!Array.isArray(notesToDelete) || notesToDelete.length === 0) {
      return res.status(400).json({ status: "error", message: "Invalid Notes Array", data: null });
    }
    ...
    if (noteIds.length === 0) {
      return res.status(404).json({ status: "error", message: "No valid note IDs provided", data: null });
    }

    const noteDelete = await Note.deleteMany({ _id: { $in: noteIds }, userId });

    if (noteDelete.deletedCount === 0) {
      return res.status(204).json({ status: "error", message: "No matching notes found!", data: null });
    }
    ...
    return res.status(200).json({ status: "success", message: "Notes deleted successfully", data: null });
  } catch (e) {
...
const getLastViewedNotes = async(req,res)=>{
  try{
   ...
   if(lastViewedNotes.length===0){
    return res.status(204).json({status:"success",message:"No last viewed notes", data:[]})
   }
...
const noteInfo = async(req,res)=>{
  try{
    ...
    const noteInfo = await Note.findOne({_id:noteId, userId}).select("noteName fileSize fileType lastViewed createdAt").populate('folderId')
...
module.exports = {uploadNotes, deleteNotes, replaceNotes,updateLastViewed,getLastViewedNotes, noteInfo}