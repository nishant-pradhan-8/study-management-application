   ...
   useEffect(()=>{
    ...
    return(
            repeatedFile && !duplicateOptionSelected && 
                ...
                <h2 className="text-xl font-semibold text-gray-100 mb-4">Upload options</h2>
                <p className="text-gray-300 mb-6">
                  <span className="font-medium text-gray-100">{repeatedFile.length===1?repeatedFile[0].name:"One or more files"}</span>{" "}
                  already exists. Do you want to replace the existing file with
                  a new version or cancel the uploading of the file?
                </p>
        
                <div className="flex justify-end mt-6 space-x-3">
                  <button onClick={()=>handleDuplicateFileOptionSelection('cancel')} className="px-4 py-2 text-gray-300 bg-gray-700 rounded-xl hover:bg-gray-600">
                    Cancel
                  </button>
                  <button onClick={()=>handleDuplicateFileOptionSelection('upload')} className="px-4 py-2 text-white bg-blue-600 rounded-xl hover:bg-blue-500">
                    Upload
                  </button>
                </div>
              </div>
