      ...
      const handlePickerOpen = (ref: React.RefObject<HTMLInputElement | null>) => {
       ...
       console.log(e.target as HTMLFormElement)
        ...
        const eventObj = eventValidation(e)
          ...
          const res = await updateEvent(eventObj.title, eventObj.start, eventObj.end, eventObj.description, editingEventId)
          if(!res.data) {
      ...
      <Accordion className="max-h-[28rem] event-update-accrodion overflow-y-scroll" type="single" collapsible>
        { events && events.length>0?(
          ...
          <div className="flex justify-between items-center border-b p-2 px-4 bg-slate-300 rounded-b-none rounded-xl">
            <h3 className="font-semibold text-black  ">Event details</h3>
            
            <button onClick={()=>setEditingEventId(event.id)} className=" flex items-center gap-2 text-black text-sm hover:underline">
              <Image src="/images/pencil.svg" alt="pencil" width={20} height={20} />
              Edit
            </button>
          </div>
          <div className="px-4 py-4 bg-slate-200 rounded-b-xl">
            <form onSubmit={handleEventUpdate} className=" flex flex-col gap-4">
            <div className="flex flex-row gap-4 text-black items-center ">
              <label htmlFor="title" className="text-[1rem]">Title:</label>
              <input name="eventName" id="title" type="text" readOnly={!(editingEventId===event.id)} 
               onChange={(e) =>
                setEventDetails((val) => ({
                  ...val,
                  title: e.target.value,
                }))
              } value={editingEventId===event.id?eventDetails.title:event.title} className={`p-2 px-4 w-full rounded-xl border-black border-[1px] ${editingEventId===event.id?'bg-slate-100':'bg-slate-300'} outline-none `} />
            </div>
            <div className="flex flex-row gap-4 text-black items-center relative">
                            <Label htmlFor="start-date">Start Date:</Label>
                            <input
                           value={editingEventId===event.id?eventDetails.start:event.start}
                              ref={startDateRef}
                              type="datetime-local"
                              id="start-date"
                              name="startDate"
            ...
            <div className="flex flex-row gap-4 text-black items-center relative">
                            ...
                            <input
                           value={editingEventId===event.id?eventDetails.end:event.end}
                           onChange={(e) =>
                            setEventDetails((val) => ({
                              ...val,
                              end: e.target.value,
            ...
            <div className="flex flex-row gap-4 text-black items-center ">
              <label htmlFor="description" className="text-[1rem]">Description:</label>
              <textarea
                onChange={(e) =>
                  setEventDetails((val) => ({
                    ...val,
                ...
                id="description" name="description" readOnly={!(editingEventId===event.id)} value={editingEventId===event.id?eventDetails.description:event.description} className={`p-2 max-h-[4rem] px-4 border-black border-[1px] rounded-xl w-full ${editingEventId===event.id?'bg-slate-100':'bg-slate-300'} outline-none `} />
            </div>

            {
            editingEventId===event.id && <div className="flex flex-row justify-end gap-4">
                <button type="button" onClick={()=>setEditingEventId(null)} className="primary-btn bg-slate-300  text-black font-normal">Cancel</button>
            <button type="submit"   className="primary-btn bg-slate-300  text-black font-normal">Update</button>
